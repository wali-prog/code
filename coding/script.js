...
document.getElementById("forwardButton").addEventListener("click", function () {
  // Start sliding the canvas to the left
  let slideDuration = 1000; // in milliseconds (1 second)
  let startTime = null;

  function slideAway(timestamp) {
    if (!startTime) startTime = timestamp;
    let progress = timestamp - startTime;

    // Calculate the canvas position, moving it left
    let translateX = Math.max(-w * (progress / slideDuration), -w);
    ctx.setTransform(1, 0, 0, 1, translateX, 0); // Apply the translation

    // Redraw the canvas with the new position
    ctx.clearRect(0, 0, w, h);
    anim(); // continue the animation

    if (progress < slideDuration) {
      requestAnimationFrame(slideAway); // Continue sliding
    } else {
      // Redirect to the new page once the slide is done
      window.location.href = "next.html"; // Put the URL of your next page here
    }
  }

  requestAnimationFrame(slideAway); // Start the sliding animation
});
